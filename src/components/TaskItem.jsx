...
function TaskItem(props) {
	const { text, completed, id } = props.task;
	const classes = ["todo__list-item", "list-item"];

	if (completed) {
		classes.push("list-item--check");
	}

	return (
		<li className={classes.join(" ")}>
			<div className='list-item__check'>
				<div className='list-item__check-label' onClick={() => props.toggleTask(id)}>
					{text}
				</div>
				<span>
					<svg
			...
			<button className='list-item__remove' onClick={() => props.removeTask(id)}>
				<svg
					width='11'
					height='11'
					viewBox='0 0 11 11'
